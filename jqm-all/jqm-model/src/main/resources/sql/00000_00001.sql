...
/* All key/values of the different configuration relations go here */
CREATE MEMORY TABLE __T__CONFIGURATION_PARAMETER
(
    ID INTEGER NOT NULL,
    ...
    CONSTRAINT UK_CONFIGURATION_PARAMETER_1 UNIQUE(ITEM, ITEM_TYPE, KEYNAME)
);
CREATE INDEX IDX_FK_CL_CONFIGURATION_PARAMETER_1 ON __T__CONFIGURATION_PARAMETER(ITEM, ITEM_TYPE);
...
(
    ID INTEGER NOT NULL,
    ENABLED BOOLEAN, /* TODO: remove - put in action queue */
...
CREATE MEMORY TABLE __T__RM
(
    ID INTEGER NOT NULL,
    IMPLEMENTATION VARCHAR(254) NOT NULL,
    RM_KEY VARCHAR(254),
    DESCRIPTION VARCHAR(1000),
    LAST_MODIFIED TIMESTAMP NOT NULL,
    ENABLED BOOLEAN,
    
    CONSTRAINT PK_RM PRIMARY KEY(ID)
);

CREATE MEMORY TABLE __T__RM_POLLER_MAPPING
(
    ID INTEGER NOT NULL,
    RM INTEGER NOT NULL,
    POLLER INTEGER NOT NULL,
    LAST_MODIFIED TIMESTAMP NOT NULL,
    
    CONSTRAINT PK_RM_POLLER_MAPPING PRIMARY KEY(ID),
    CONSTRAINT FK_RM_POLLER_MAPPING_1 FOREIGN KEY(RM) REFERENCES __T__RM(ID),
    CONSTRAINT FK_RM_POLLER_MAPPING_2 FOREIGN KEY(POLLER) REFERENCES __T__QUEUE_NODE_MAPPING(ID)
);
CREATE INDEX IDX_FK_RM_POLLER_MAPPING_1 ON __T__RM_POLLER_MAPPING(RM);
CREATE INDEX IDX_FK_RM_POLLER_MAPPING_2 ON __T__RM_POLLER_MAPPING(POLLER);

CREATE MEMORY TABLE __T__RM_NODE_MAPPING
(
    ID INTEGER NOT NULL,
    RM INTEGER NOT NULL,
    NODE INTEGER NOT NULL,
    ...
    CONSTRAINT PK_RM_NODE_MAPPING PRIMARY KEY(ID),
    CONSTRAINT FK_RM_NODE_MAPPING_1 FOREIGN KEY(RM) REFERENCES __T__RM(ID),
    CONSTRAINT FK_RM_NODE_MAPPING_2 FOREIGN KEY(NODE) REFERENCES __T__NODE(ID)
);
CREATE INDEX IDX_FK_RM_NODE_MAPPING_1 ON __T__RM_NODE_MAPPING(RM);
CREATE INDEX IDX_FK_RM_NODE_MAPPING_2 ON __T__RM_NODE_MAPPING(NODE);
