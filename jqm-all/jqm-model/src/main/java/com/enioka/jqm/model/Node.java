...
public class Node implements Serializable
{
    ...
    /**
     * See {@link #getJobRepoDirectory()}
     */
    public void setJobRepoDirectory(final String jobRepoDirectory)
    {
     ...
     */
    public static List<Node> getNodes(DbConn cnx)
    {
        return select(cnx, "node_select_all");
    }

    /**
     * @return Node corresponding to id
     */
    public static Node getNode(DbConn cnx, int id)
    {
        return select_single(cnx, "node_select_by_id", id);
    }

    /**
     * @return Node corresponding to name
     */
    public static Node getNode(DbConn cnx, String name)
    {
        return select_single(cnx, "node_select_by_key", name);
    }

    public static void upsert(DbConn cnx, Node node)
    {
        if (node.getId() != null)
        {
            QueryResult qr = cnx.runUpdate("node_update_changed_by_id", node.getOutputDirectory(), node.getDns(), node.getEnabled(),
                node.getJmxRegistryPort(), node.getJmxServerPort(), node.getLoadApiAdmin(), node.getLoadApiClient(), node.getLoapApiSimple(),
                node.getName(), node.getPort(), node.getJobRepoDirectory(), node.getRootLogLevel(), node.getStop(), node.getTmpDirectory(),
                node.getId(), node.getOutputDirectory(), node.getDns(), node.getEnabled(), node.getJmxRegistryPort(), node.getJmxServerPort(),
                node.getLoadApiAdmin(), node.getLoadApiClient(), node.getLoapApiSimple(), node.getName(), node.getPort(),
                node.getJobRepoDirectory(), node.getRootLogLevel(), node.getStop(), node.getTmpDirectory());

            if (qr.nbUpdated != 1)
            {
                throw new NoResultException("No update was done as object either does not exist or no modifications were done");
            }
        }
        else
        {
            // Should actually never be used... nodes should be created through CLI.
            Node.create(cnx, node.getName(), node.getPort(), node.getOutputDirectory(), node.getJobRepoDirectory(), node.getTmpDirectory(),
                node.getDns(), node.getRootLogLevel());
        }
    }
            ...
            String logLevel)
    {
        ...
        res.dns = dns;
        res.port = port;
        res.outputDirectory = dlRepo;
        res.tmpDirectory = tmpDir;
        res.jobRepoDirectory = repo;
    ...
    static Node map(DbConn cnx, ResultSet rs, int colShift)
    {
        try
        {
            ...
            tmp.outputDirectory = rs.getString(2 + colShift);
            ...
            tmp.jobRepoDirectory = rs.getString(12 + colShift);
